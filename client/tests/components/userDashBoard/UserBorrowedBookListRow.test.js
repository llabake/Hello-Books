...
import {
  ...
  mapDispatchToProps,
  mapStateToProps
} from '../../../components/userDashBoard/UserBorrowedBookListRow';
...
function setup() {
  ...
  const wrapper = shallow(<UserBorrowedBookListRow {...props} />);
...
describe('Book Return list row', () => {
  ...
  test('should handle return request accept action', () => {
    ...
    props.returnBookAction.mockReset();
    wrapper.instance().handleReturn(event)
    expect(props.returnBookAction).toHaveBeenCalled()
  });
  ...
  test('mapDispatchToProps', () => {
    ...
    expect(mapDispatchToProps(dispatch)).toHaveProperty('returnBookAction');

    const { returnBookAction } = mapDispatchToProps(dispatch);
    returnBookAction();

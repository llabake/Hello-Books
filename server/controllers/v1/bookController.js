...
export const borrowBook = (req, res) => {
    ...
    try {
        if(!Book.isAvailable(bookId)){
            return res.status(400).json({message: 'book currently not available for borrow'})        
        } else {
            if (BorrowedBook.existsByUserIdAndBookId(userId, bookId)) {
                return res.status(409).json({message: "You have made this request earlier!"});
            } else {
...
export const returnBook = (req, res) => {
    ...
    try {
        ...
        if(borrowedBook.borrowedStatus === 'accepted') {
            ...
            return res.status(400).json({message:'Request to borrow is still pending'})    
        }
    } catch (error) {
        return res.status(400).json({error})
    }
